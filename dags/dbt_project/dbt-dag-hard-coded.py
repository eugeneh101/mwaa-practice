...
bash_op2 = BashOperator(
    ...
    bash_command=f"{MWAA_DBT_PREFIX_COMMAND} && dbt debug --profiles-dir profile_hard_coded/",  # --log-path CLI doesn't seem to work
    dag=dag,
)
bash_op3 = BashOperator(
    task_id="dbt3",  ### have to manually click to create the Redshift database
    bash_command=f"{MWAA_DBT_PREFIX_COMMAND} && dbt run --profiles-dir profile_hard_coded/",
    # bash_command=" ".join([
...
bash_op4 = BashOperator(
    ...
    bash_command=f"{MWAA_DBT_PREFIX_COMMAND} && dbt test --profiles-dir profile_hard_coded/",
    dag=dag,
)
